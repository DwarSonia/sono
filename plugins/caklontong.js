...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    if (id in conn.caklontong) {
        conn.reply(m.chat, 'Ainda há perguntas sem resposta neste chat', conn.caklontong[id][0])
        throw false
    }
...
Modelo ${usedPrefix}calo untuk bantuan
Bonus: ${poin} XP
`.trim()
    conn.caklontong[id] = [
