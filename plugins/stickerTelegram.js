...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (!args[0]) throw `*Este comando para recuperar adesivos do Telegram*\n\nExemplo:\n${usedPrefix + command} https://t.me/addstickers/menggokil`
    if (!args[0].match(/(https:\/\/t.me\/addstickers\/)/gi)) throw `*Link errado! Este comando é para recuperar stickers do Telegram*\n\nExemplo:\n${usedPrefix + command} https://t.me/addstickers/menggokil`
    ...
    m.reply(`*Total:* ${json.result.length}\n*Conclusão estimada:* ${json.result.length * 2} segundo\n\n` + mes)
