...
module.exports = {
    async before(m) {
        let id = m.chat
        if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Modelo.*calo/i.test(m.quoted.contentText)) return !0
        this.caklontong = this.caklontong ? this.caklontong : {}
        if (!(id in this.caklontong)) return m.reply('O assunto acabou')
        if (m.quoted.id == this.caklontong[id][0].id) {
            ...
            if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
                ...
                await this.sendButton(m.chat, `*Correto!* +${this.caklontong[id][2]} XP\n${json.deskripsi}`, author, 'Cak Lontong', '.caklontong', m)
