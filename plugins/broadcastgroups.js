let handler  = async (m, { conn, text }) => {
  ...
  conn.reply(m.chat, `_	Envie uma mensagem de difusão para ${groups.length} grup_`, m)
  for (let id of groups) await conn.copyNForward(id, conn.cMod(m.chat, cc, /bc|broadcast/i.test(teks) ? teks : teks + '\n' + readMore + '「 Todos os grupos de transmissão 」\n' + randomID(32)), true).catch(_=>_)
  m.reply('Concluído Transmitir todo o grupo:)')
}
